    ...
    for( int n = 0; n < draw_data->CmdListsCount; n++ )
    {
        ...
        for( int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++ )
        {
            ...
            else
            {
                ...
                VkDescriptorSet descriptor_set ={ ( VkDescriptorSet )pcmd->TextureId };
                vkCmdBindDescriptorSets( command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, &descriptor_set, 0, NULL );
...
void ImGui_ImplVulkanH_CreateWindowSwapChain( VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count )
{
    ...
    // Create the Render Pass
    {
        ...
        colorAttachmentDesc.format = wd->SurfaceFormat.format;
        colorAttachmentDesc.samples = VK_SAMPLE_COUNT_1_BIT;
        colorAttachmentDesc.loadOp = wd->ClearEnable ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
