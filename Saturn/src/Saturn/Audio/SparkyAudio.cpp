...
namespace Saturn {
	...
	AudioSource Audio::LoadAudioSourceOgg(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			SAT_CORE_INFO("File Info - {0} {1}", filename, ":");
			SAT_CORE_INFO("  Channels: {0}", channels);
			SAT_CORE_INFO("  Sample Rate: {0}", sampleRate);
			SAT_CORE_INFO("  Expected size: {0}", bufferSize);
		}
		...
		if (s_DebugLog)
			SAT_CORE_INFO("  Read {0} {1}", size, " bytes");
		...
		if (alGetError() != AL_NO_ERROR)
			SAT_CORE_ERROR("Failed to setup sound source");
	...
	AudioSource Audio::LoadAudioSourceMP3(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			SAT_CORE_INFO("File Info - {0}", filename);
			SAT_CORE_INFO("  Channels: {0}", channels);
			SAT_CORE_INFO("  Sample Rate: {0}", sampleRate);
			SAT_CORE_INFO("  Size: {0} {1} ", size, "bytes");

			auto [mins, secs] = result.GetLengthMinutesAndSeconds();
			SAT_CORE_INFO("  Length: {0} {1} {3} {4} ", mins , "m" , secs ,"s");
		}
	...
	static void PrintAudioDeviceInfo()
	{
		if (s_DebugLog)
		{
			SAT_CORE_INFO("Audio Device Info:");
			SAT_CORE_INFO("  Name: {0} ", s_AudioDevice->DeviceName);
			SAT_CORE_INFO("  Sample Rate: {0}", s_AudioDevice->Frequency);
			SAT_CORE_INFO("  Max Sources: {0}",  s_AudioDevice->SourcesMax);
			SAT_CORE_INFO("    Mono: {0}", s_AudioDevice->NumMonoSources);
			SAT_CORE_INFO("    Stereo: {0}", s_AudioDevice->NumStereoSources);
		}
	}
