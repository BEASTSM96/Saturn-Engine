...
#define SAT_ASSERT_NO_MESSAGE(condition) { if(!(condition)) { /*SAT_ERROR("Assertion Failed");*/ __debugbreak(); } }
#define SAT_ASSERT_MESSAGE(condition, ...) { if(!(condition)) { /*SAT_ERROR("Assertion Failed: {0}", __VA_ARGS__);*/ __debugbreak(); } }
