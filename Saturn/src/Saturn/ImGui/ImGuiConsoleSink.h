...
namespace Saturn {
	...
	public:
		explicit ImGuiConsoleSink(bool forceFlush = false, uint8_t bufferCapacity = 10)
			: m_MessageBufferCapacity(forceFlush ? 1 : bufferCapacity), m_MessageBuffer(std::vector<std::shared_ptr<ImGuiConsole::Message>>(forceFlush ? 1 : bufferCapacity))
		{
		}
	...
	protected:
		...
		{
			for (std::shared_ptr<ImGuiConsole::Message> message : m_MessageBuffer)
	...
	private:
		...
		std::vector<std::shared_ptr<ImGuiConsole::Message>> m_MessageBuffer;
	};
