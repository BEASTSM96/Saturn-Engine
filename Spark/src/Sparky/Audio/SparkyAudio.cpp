...
namespace Sparky {
	...
	AudioSource Audio::LoadAudioSourceOgg(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			SP_CORE_INFO("File Info - {0} {1}", filename, ":");
			SP_CORE_INFO("  Channels: {0}", channels);
			SP_CORE_INFO("  Sample Rate: {0}", sampleRate);
			SP_CORE_INFO("  Expected size: {0}", bufferSize);
		}
		...
		if (s_DebugLog)
			SP_CORE_INFO("  Read {0} {1}", size, " bytes");
	...
	AudioSource Audio::LoadAudioSourceMP3(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			SP_CORE_INFO("File Info - {0}", filename);
			SP_CORE_INFO("  Channels: {0}", channels);
			SP_CORE_INFO("  Sample Rate: {0}", sampleRate);
			SP_CORE_INFO("  Size: {0} {1} ", size, "bytes");

			auto [mins, secs] = result.GetLengthMinutesAndSeconds();
			SP_CORE_INFO("  Length: {0} {1} {3} {4} ", mins , "m" , secs ,"s");
		}
	...
	static void PrintAudioDeviceInfo()
	{
		if (s_DebugLog)
		{
			SP_CORE_INFO("Audio Device Info:");
			SP_CORE_INFO("  Name: {0} ", s_AudioDevice->DeviceName);
			SP_CORE_INFO("  Sample Rate: {0}", s_AudioDevice->Frequency);
			SP_CORE_INFO("  Max Sources: {0}",  s_AudioDevice->SourcesMax);
			SP_CORE_INFO("    Mono: {0}", s_AudioDevice->NumMonoSources);
			SP_CORE_INFO("    Stereo: {0}", s_AudioDevice->NumStereoSources);
		}
	}
